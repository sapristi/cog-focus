...
`cog-focus/config.yaml` supports an `autocommit` flag (default `false`). When set to `true`, Claude commits completed work automatically — either when a subtask is checked off in `roadmap.md` or when a coherent unit of work is done. Commits run via a sub-agent that:
