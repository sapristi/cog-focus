...
- **Plugin changes don't propagate to live instances — build an explicit sync step.** Updating an onboarding skill, observation format, or hot-memory rule helps *future* projects but leaves existing cog-focus instances stale. Either (a) re-run the onboarding flow on the live instance, (b) add a sync step to /housekeeping (cf. "sync memory headers from plugin template"), or (c) accept and document the drift. Silent expectation that templates propagate has misfired three times so far.
